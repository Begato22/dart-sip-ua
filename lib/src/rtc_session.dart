...
// إصلاح SDP لـ 3CX قبل التعيين
    print('[3CX PATCH] 🔧 Fixing 3CX SDP in _processInDialogSdpOffer()...');
    String fixedSdp = _fix3CXSdp(offer.sdp!);
    RTCSessionDescription fixedOffer = RTCSessionDescription(fixedSdp, offer.type);
...
// إصلاح SDP لـ 3CX قبل التعيين
      print('[3CX PATCH] 🔧 Fixing 3CX SDP in _receiveInviteResponse()...');
      String fixedSdp = _fix3CXSdp(answer.sdp!);
      RTCSessionDescription fixedAnswer = RTCSessionDescription(fixedSdp, answer.type);
...
// We created a SDP 'answer' for it, so check the current signaling state.
      ...
      try {
        await _connection!.setRemoteDescription(fixedAnswer);
  ...
  void _toggleMuteVideo(bool mute) {
    if (_localMediaStream != null) {
      if (_localMediaStream!.getVideoTracks().isEmpty) {
        logger.w('Went to toggle mute video but local stream has no video tracks');
      }
  ...
  String _fix3CXSdp(String sdp) {
    // إذا كان SDP من 3CX ولا يحتوي على DTLS fingerprint، أضف واحداً وهمياً
    if (sdp.contains('RTP/SAVP') && !sdp.contains('a=fingerprint:')) {
      ...
      print('[3CX PATCH] ✅ Added DTLS fingerprint to SDP');
    }

    // تأكد من وجود crypto line لـ SRTP
    if (!sdp.contains('a=crypto:') && sdp.contains('RTP/SAVP')) {
      ...
      print('[3CX PATCH] ✅ Added crypto line to SDP');
    }
